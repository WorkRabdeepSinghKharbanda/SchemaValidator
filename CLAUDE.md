...
- **Export report / export docs** (`App.tsx`'s `handleExportReport`, `lib/docgen.ts`'s `generateSchemaDocs`) — downloads validation results as JSON, or a Markdown field table generated from the schema, respectively.
