...
- **Core validate** (`lib/parse.ts`, `lib/validate.ts`) — see control-flow diagram above. `validate()` takes an options object (`sourceText`, `sourceFormat`, `draft`, `schemaCacheKey`, `refSchemas`) rather than positional params — extend that object, don't add more positional args. Error-to-line mapping (`locateNode`) unescapes each ajv `instancePath` segment (`unescapeJsonPointerSegment`: `~1`→`/`, then `~0`→`~`, per RFC 6901) before comparing against the parse tree's real property names — without this, a property literally named e.g. `"a/b"` (which ajv reports as path segment `a~1b`) would never resolve to a line number, since the parse tree stores the actual unescaped name.
...
- **Batch validation mode** (`App.tsx`'s `runBatchValidation`, `BatchTable`) — validates each item of a top-level data array independently, shown as a pass/fail table with a progress bar. Must mirror single mode's "empty schema → trivially valid" behavior (see control-flow note above). Toggling batch mode on/off clears `dataMarkers`/`errors`/`success`/`batchRows` immediately — without that, stale single-mode error squiggles used to linger in the data editor after switching into batch mode. **This clearing lives in one place, `handleToggleBatchMode()`** — both the Toolbar checkbox and the command palette's "Toggle batch mode" entry call it. They used to be two separate inline closures, and the palette's copy was missing the clearing entirely (a real bug, found by a full-codebase review) — if you add a third way to toggle batch mode, route it through this function too, don't inline `setBatchMode` again.

- **Restoring history/a workspace clears prior validation output** (`App.tsx`'s `clearValidationState()`, called from both `handleRestoreHistory` and `handleRestoreWorkspace`) — resets `success`/`errors`/`dataMarkers`/`batchRows`/`jumpLine`/`canAutoFix`/`lastValidData`/`showDiff`/`schemaDiffTarget`. Without this, a restore could leave a stale Monaco error squiggle attached to an unrelated line of the newly-restored data (markers are re-filtered by the new document's line count, so they don't just disappear — they can land on the wrong line), or leave "Show diff from last valid" comparing against a completely different session's data. Same bug class as the batch-mode one above; if you add a third "swap out schemaText/dataText wholesale" transition, call `clearValidationState()` there too.
...
- **History** (`lib/history.ts`) — localStorage list of past validations, capped at 20, only written on an explicit Validate/Cmd+S (never from realtime auto-validate, to avoid spamming entries). Stores `{schema, data, format, draft, refSchemas, valid}`; `loadHistory()` defaults missing `draft`/`refSchemas` on entries saved before those fields existed, and — like `workspaces.ts`/`share.ts` — validates `format`/`draft` against `isFormat()`/`isDraft()` rather than only defaulting on `undefined`, dropping an entry with an invalid `format` and defaulting an invalid `draft` to `"2020-12"`. All three persistence surfaces (`history.ts`, `workspaces.ts`, `share.ts`) should use the same `isFormat`/`isDraft` guards for the same fields — this drifted once (a corrupted/hand-edited `draft` value could pass straight through `workspaces.ts`'s `importWorkspacesBackup` and crash `compile()` with a confusing "AjvClass is not a constructor" instead of a clean rejection) and was fixed by adding `isDraft()` (in `lib/validate.ts`, next to `DRAFTS`) as the single source of truth, mirroring `isFormat()` in `parse.ts`.
